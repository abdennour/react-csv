...
export const string2csv = ((data, headers, separator, newLineSeparator = '\n') =>
  (headers) ? `${headers.join(separator)}${newLineSeparator}${data}`: data
);
...
export const toCSV = (data, headers, separator, enclosingCharacter, newLineSeparator) => {
 ...
 if (typeof data ==='string') return string2csv(data, headers, separator, newLineSeparator);
